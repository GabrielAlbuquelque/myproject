...
<p>	<?php
	...
	else if ($_REQUEST ['alterar']=="endereco"){
		$endereco	=$_REQUEST['valor'];
		if ($endereco==''){
			echo "Insira um endereço válido!";
			$flag=false;
		}
		else{
			$query="UPDATE usuarios 
	...
	else if ($_REQUEST ['alterar']=="telefone"){
		$telefone	=$_REQUEST['valor'];
	...
	else if ($_REQUEST ['alterar']=="email"){
		$email	=$_REQUEST['valor'];
		if ($email==''){
	...
	if ($flag==true){
		...
		echo "<form action=\"altcadastro.php\" method=\"post\">";
